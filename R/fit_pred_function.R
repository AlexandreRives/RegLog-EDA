...
#'
fit_reg_log <- function(formula, data, mode, batch_size, normalize = FALSE, learning_rate = 0.1, max_iter = 100, ncores = 1, graph){
  ...
  if (mode == "batch"){
    ...
    # }
    coefs <- batch_gradient_descent(df, colnames(X), colnames(y), learning_rate, max_iter, graph) ; coefs
  } else if (mode == "online"){
    ...
    # }
    coefs <- online_stochastic_gradient_descent(df,colnames(X),colnames(y),learning_rate,max_iter, graph)
    ...
    # }
    coefs <- gradient_mini_batch(df,colnames(X),colnames(y),batch_size,learning_rate, max_iter, graph)
  }
  stopCluster(cl)
...
# tic()
# obj <- fit_reg_log(recode~., data=breast, mode="batch", normalize = TRUE, learning_rate =0.1 , max_iter = 50, ncores = 1 , graph = TRUE) ; obj
# toc()
# obj <- fit_reg_log(recode~., data=breast, mode="online", normalize = TRUE, learning_rate =0.1 , max_iter = 20, ncores = 1 , graph = TRUE) ; obj
# obj <- fit_reg_log(recode~., data=breast, mode="mini_batch", batch_size = 200, normalize = TRUE, learning_rate =0.1 , max_iter = 50, ncores = 1 , graph = TRUE) ; obj
# summary(obj)
# print(obj)
